...
// On change: 1-second debounce -> invalidate rule provider cache ->
...
public:
    ...
    ConfigWatcher(std::string rulesPath, amsi_ipc::IAmsiRuleProvider* ruleProvider);
    ~ConfigWatcher();

    void Start();
    void Stop();
    void InvalidateRuleCacheForTest();

private:
    std::string m_rulesPath;
    std::string m_rulesDir;
    amsi_ipc::IAmsiRuleProvider* m_ruleProvider;
