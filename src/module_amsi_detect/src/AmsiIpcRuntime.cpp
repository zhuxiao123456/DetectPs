...
namespace Engine {
    ...
    bool AmsiIpcRuntime::Start(const AmsiDetect::AmsiRuleSnapshot &snapshot, std::string &error) {
         ...
         * host->dll
         * - DLL 连接后不是马上写请求；
          - DLL 连接后会阻塞等 Host 写 1 字节控制信号；
          - Host 必须保存这些已连接 DLL 的 handle；
          - 等到 reload/unload/pause/resume 时，Host 遍历 clients_，逐个写信号；
          - 写完后关闭 handle，DLL 再重连等待下一次通知
         */
    ...
    /**
     * amsi dll升级或者卸载时，对当前状态进行备份，确保升级过程不会系统卡死、漏报或误报
