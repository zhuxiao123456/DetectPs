...
bool HostGuardDemoApp::SetRequiredDllHash(const std::string& hash)
{
    if (!ruleProvider_) {
        startError_ = "rule provider is not initialized";
        return false;
    ...
    ruleProvider_->SetRequiredDllHash(hash);

    if (amsiIpcModule_) {
        std::string error;
        if (!amsiIpcModule_->ReloadRules(1000, error)) {
            startError_ = "HostGuardAmsiIpcModule dll hash update failed: " + error;
            return false;
        }
    }
    return true;
...
void HostGuardDemoApp::PrintStatus(std::ostream& output) const
{
    output << "strictHostGuardMode: " << (options_.strictHostGuardMode ? "yes" : "no") << '\n'
           ...
           << "requiredDllHash: "
           << (ruleProvider_ ? ruleProvider_->required_dll_hash() : std::string("not-ready")) << '\n'
           << "providerCacheReady: "
