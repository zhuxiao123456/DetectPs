#pragma once
// =========================================================================
// rasp_lua_engine.h — Shared Lua 5.4 sandbox for native RASP modules.
//
...
//     Scripts see only what was pushed — no cross-module leakage.
...
// Lua scripts only see the keys that were pushed — no cross-module field
// leakage, no nil-access errors from fields the module never set.
struct RaspLuaField
{
    std::string name;
    std::string value;
    bool        isBinary = false;  // true → lua_pushlstring (preserves null bytes in body)
};
...
// AMSI: [](const char* m){ RaspLog("%s", m); }          (ring buffer → IPC)
